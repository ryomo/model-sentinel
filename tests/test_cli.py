...
class TestCLI(unittest.TestCase):
    ...
    def test_hf_model_not_verified(self, mock_verify_hf):
        ...
        self.assertTrue(
            any(
                "Repository test/repo at revision main is not verified." in call
    ...
    def test_local_model_verified(self, mock_verify_local):
        ...
        print_calls = [call[0][0] for call in mock_print.call_args_list]
        self.assertTrue(
            any("Verifying local model: /path/to/model" in call for call in print_calls)
        )
    ...
    def test_local_model_not_verified(self, mock_verify_local):
        ...
        self.assertTrue(
            any(
                "Local model /path/to/model is not verified." in call
                for call in print_calls
            )
        )
        self.assertTrue(
