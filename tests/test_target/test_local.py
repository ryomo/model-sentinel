...
class TestTargetLocal(unittest.TestCase):
    ...
    def test_detect_model_changes_no_changes(self):
        ...
        with patch.object(
            self.target, "_calculate_directory_hash", return_value=test_hash
        ):
            with patch(
    ...
    def test_detect_model_changes_with_changes(self):
        ...
        with patch.object(
            self.target, "_calculate_directory_hash", return_value=new_hash
        ):
            with patch(
                "model_sentinel.target.base.TargetBase.check_model_hash_changed",
                return_value=True,
    ...
    def test_verify_local_files_failure(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                self.target, "_verify_files_workflow", return_value=False
            ):
                with patch("builtins.print"):
                    result = self.target.verify_local_files(temp_model_dir)
...
class TestVerifyLocalModel(unittest.TestCase):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with patch("builtins.print"):
                result = verify_local_model(
                    temp_model_dir, gui=False, exit_on_reject=False
                )
    ...
    def test_verify_local_model_gui_success(self, mock_target_class):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "model_sentinel.target.local._handle_gui_verification",
                return_value=True,
            ) as mock_gui:
                with patch("builtins.print"):
    ...
    def test_verify_local_model_gui_import_error(self, mock_target_class):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with patch(
                ...
                with patch("builtins.print"):
                    result = verify_local_model(
                        temp_model_dir, gui=True, exit_on_reject=False
                    )
    ...
    @patch("model_sentinel.target.local.TargetLocal")
    def test_verify_local_model_gui_closed_with_exit_on_reject_true(
            ...
            with patch(
                ...
                with patch("builtins.print"):
                    with patch("builtins.exit") as mock_exit:
                        result = verify_local_model(
                            temp_model_dir, gui=True, exit_on_reject=True
                        )
    ...
    @patch("model_sentinel.target.local.TargetLocal")
    def test_verify_local_model_gui_closed_with_exit_on_reject_false(
        self, mock_target_class
    ):
